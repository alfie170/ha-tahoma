...
class TahomaClient:
    ...
    async def login(self, register_event_listener: Optional[bool] = True) -> bool:
        ...
        """
        authPayload = {"username": self.username, "password": self.password, "grant_type":'password',
                       "client_id": '0d8e920c-1478-11e7-a377-02dd59bd3041_1ewvaqmclfogo4kcsoo0c8k4kso884owg08sg8c40sk4go4ksg',"client_secret":'12k73w1n540g8o4cokg0cw84cog840k84cwggscwg884004kgk'}
        res = await self.__auth(data=authPayload)
    ...
    async def __auth(
        self, payload: Optional[JSON] = None, data: Optional[JSON] = None
    ) -> Any:
        """ Make a POST request to the TaHoma API """
        url = 'https://accounts.somfy.com/oauth/oauth/v2/token'
        headers = {'User-Agent': 'TaHoma/15014 CFNetwork/1120 Darwin/19.0.0', 'Content-Type': 'application/x-www-form-urlencoded'}
        async with self.session.post(url, data=data, json=payload, headers=headers
        ) as response:
            await self.check_response(response)
            return await response.json()
